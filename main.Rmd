...
# Defino est_exp
est_exp <- function(x) { b_exp_orig * exp(a_exp*x)}
```

`est_exp` es equivalente, entonces al modelo ajustado
...
Sin ser un experto en estimación poblacional, podemos observar que en general, ninguno de los modelos provee una aproximación demasiado válida de la evolución de la población. Sin embargo, de entre las opciones consideradas, **el modelo con menor error total es el modelo lineal**, y por lo tanto lo consideraremos el más apropiado.